...
| 규칙 + LLM (OpenAI `gpt-5.5`, 4회차) | 25 | 1 | 0 |

LLM은 보류한 5개에서만 쓰입니다. 4회차는 25개로 가장 좋았지만, 4번 돌리는 동안 **가를 근거가 없는 두 구간의 답이 매번 바뀌었습니다** ([아래](#llm-판단)).
...
- **합성 부분 유실**은 4회 모두 틀렸습니다. 프롬프트의 "주기 신호가 워크로드라면 워크로드가 멈춘 것일 수도 있다"는 문장이 healthcheck가 멈춘 경우에도 적용된 것으로 보입니다.
- 1–3회차는 샘플을 순수 Tetragon 로그로 다시 만들기 전이고, 4회차는 다시 만든 샘플과 26개 구간 기준입니다. 5개 구간의 판정 내용은 같습니다.
- 답이 흔들리는 걸 막으려고 `--votes`를 넣었습니다. 같은 질문을 N번 해서 80% 이상 같은 답일 때만 판정하고, 갈리면 `UNSURE`로 보류합니다. `python3 eval.py --llm --votes 5`로 잴 수 있으며, **아직 실제 LLM으로 측정하지 않았습니다**.

4회차의 전체 출력은 [eval_llm.txt](eval_llm.txt)에 있습니다.
...
- 수집기는 이벤트를 봤지만 로그 보관 단계(회전, 용량 제한)에서 지워진 경우는 잡지 못합니다. 4번 실험에서 실제로 일어났습니다.
