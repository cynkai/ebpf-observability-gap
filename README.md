...
| 규칙 + LLM (OpenAI `gpt-5.5`, 한 번 질문, 4회차) | 25 | 1 | 0 |
| 규칙 + LLM (OpenAI `gpt-5.5`, 5번 질문 투표) | 24 | 1 | 1 |

LLM은 보류한 5개에서만 쓰입니다. 한 번씩 물으면 **가를 근거가 없는 두 구간의 답이 실행마다 바뀌었고**, 5번 물어 투표하자 흔들리던 구간은 보류로 바뀌었지만 **일관되게 틀리는 구간은 그대로 틀렸습니다** ([아래](#llm-판단)).
