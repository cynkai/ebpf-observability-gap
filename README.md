...
"문장 뺌"은 프롬프트에서 "주기 신호가 워크로드라면 워크로드가 멈춘 것일 수도 있다"는 문장을 뺀 결과이고, "문장 복원"은 다시 넣고 잰 결과입니다(현재 코드).

| 구간 | 정답 | 한 번 질문 1–4회차 | 투표 1회차 | 투표, 문장 뺌 | 투표, 문장 복원 |
|---|---|---|---|---|---|
| 합성: 드롭 신호 없는 전체 블랙아웃 | 관측 안 됨 | ✓✓✓✓ | ✓ | ✓ | ✓ |
| 합성: healthcheck만 사라진 부분 유실 | 관측 안 됨 | ✗✗✗✗ | ✗ | ✗ | ✗ |
| Tetragon: 막 켜졌고 워크로드 시작 전 | 관측됨 | ✓✓✓✓ | ✓ | ✓ | ✓ |
| Tetragon: 워크로드를 멈춘 조용한 40초 | 관측됨 | ✗✗✓✓ | – 보류 | ✗ (80% 이상 일치) | – 보류 |
| Tetragon: 수집기 삭제 | 관측 안 됨 | ✓✓✗✓ | ✓ | ✓ | ✓ |
| **전체 (규칙+LLM)** | | | 24/26 맞힘, 1 틀림, 1 보류 ¹ | 33/35 맞힘, 2 틀림 | **33/35 맞힘, 1 틀림, 1 보류** |

¹ 이때는 26개 구간 시절입니다. 이후 추가된 9개 구간은 규칙만으로 모두 맞혀 LLM에게 가지 않습니다.
"투표 1회차"와 "문장 복원"은 같은 프롬프트이고, 5개 구간의 판정이 똑같이 나왔습니다.
...
- **문장을 뺀 결과**: 부분 유실은 고쳐지지 않았고, 오히려 "워크로드 중지" 구간을 80% 이상 일치로 "관측 안 됨"이라고 틀렸습니다(이전에는 답이 갈려 보류). 이 문장은 워크로드가 멈춘 경우를 제대로 보게 돕고 있었던 것이라 **다시 넣었고**, 다시 잰 결과가 투표 1회차와 똑같이 나왔습니다.

마지막 실행(투표, 문장 복원)의 전체 출력은 [eval_llm.txt](eval_llm.txt)에 있습니다.
